...
def _build_scene_review_prompt(chapter: ChapterScene) -> tuple[str, str]:
    ...
    lines.append("- Each shot focuses on ONE concrete OBJECT or ACTION from the sentences")
    lines.append("  The focus field MUST name a visible thing (e.g. 'red jacket', 'phone screen',")
    lines.append("  'walking shoes', 'open suitcase'). NEVER use character names or abstractions")
    lines.append("  like 'conversation', 'Maria talking', or 'emotional moment' as focus.")
    lines.append("- VISUAL COHERENCE: If a shot has 2 sentences, BOTH must be depictable in ONE")
    lines.append("  picture showing the focus object. If sentence B talks about something the focus")
    lines.append("  image cannot show (a different object, a different place, an abstract thought),")
    lines.append("  sentence B MUST be moved to its own shot with its own focus.")
    lines.append("  Examples of BAD pairings (MUST split):")
    lines.append("    - focus='clothes on bed' + sentence about a nearby park")
    lines.append("    - focus='walking shoes' + dialogue response 'You're right, Mom!'")
    lines.append("    - focus='suitcase' + sentence 'She goes to Buenos Aires'")
    lines.append("  Having many 1-sentence shots is FINE and preferred over bad pairings.")
    lines.append("- Vary focus across consecutive shots — no two adjacent shots about the same object")
...
def _build_prompt_generation_prompt(
    ...
    lines.append("RULES:")
    lines.append("- THE OBJECT IS THE STAR. The camera points at the vocabulary object/action,")
    lines.append("  NOT at characters talking. Characters may appear in the frame but the focal")
    lines.append("  point is always the THING (suitcase, jacket, shoes, notebook, phone, door).")
    lines.append("- For dialogue shots: show what they're TALKING ABOUT. If someone says")
